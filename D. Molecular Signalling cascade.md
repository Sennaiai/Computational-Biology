...
Model and simulate the intracellular signalling cascade of dual‑target CAR‑T cells (CD19/CD22) to analyze activation, proliferation, cytotoxicity, and safety (cytokine release). The project integrates mechanistic ODE modeling with stochastic simulation and ML overlays to predict therapy efficacy and toxicity, linking directly to the T‑cell engineering data from Project 2.
...
* **Modules**:
      ...
      * **Equations**: System of ODEs (Michaelis–Menten forms) representing activation and feedback loops
...
* **Strategy**:
      ...
      * Scale activation/persistence based on CRISPR priors: PD‑1 KO -> high-activation/persistence; off‑target burden  ->  low-fitness
...
* **Metrics**:
  ...
  * Efficacy -> tumor burden AUC(low), time‑to‑clearance, CAR‑T expansion/persistence
    
  * Safety -> cytokine peak (IL‑6, IFN‑γ), activation fraction > threshold
    
* **Evaluation**: trade‑off frontiers (Pareto: efficacy-up vs toxicity-down), compare single‑ vs dual‑target
...
* **App**: sliders for antigen density, affinity, or CRISPR priors -> interactive visualization

       ## Datasets & Sources

* **Biochemical Parameters**: published CAR‑T signaling models (e.g., PI3K/AKT, NF‑κB kinetics)
  
* **Experimental References**: cytokine release assays, cell killing curves (B‑ALL)
...
* **Quantum-Enhanced Cytokine Storm Prediction:**: build a hybrid quantum-classical model using simulated and experimental cytokine data to predict storm severity. Quantum feature maps or variational circuits capture nonlinear links between T-cell activation, CAR design, and cytokine feedback to enhance early toxicity prediction and safety optimization.
